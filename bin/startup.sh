...
then
    ...
    eval "exec $wfd>&-"
    exec java $jvm_args $java9opt -classpath "${clt_dir}/libs/*:${clt_dir}/conf/" com.xjj.tools.bigdata.tunnel.ConsoleMain -I $rfd
else
    exec java $jvm_args $java9opt -classpath "${clt_dir}/libs/*:${clt_dir}/conf/" com.xjj.tools.bigdata.tunnel.ConsoleMain "$@"
fi
