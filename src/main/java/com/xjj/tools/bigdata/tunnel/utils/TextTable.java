...
public class TextTable {
    ...
    public static void main(String[] args) {
        List<String> columnNameListTmp = Arrays.asList("ServerName", "StartTime", "IsRunning");
        List<List<String>> columnValueListTmp = new ArrayList<>();
        columnValueListTmp.add(Arrays.asList("科目一","科目","式"));
        TextTable table = new TextTable(columnNameListTmp , columnValueListTmp );
    ...
    public TextTable(List<String> columnNameListTmp, List<List<String>> columnValueListTmp,
                     Map<String, Integer> maxFieldValueLengthMapTmp) {
        ...
        for (int i = 0; i < columnNameList.size(); i++) {
            if (i == 0) {
                sb.append(columnInterval + " "
                        + makeValueAlign(columnNameList.get(i), maxFieldValueLengthMap.get(columnNameList.get(i))));
            } else {
                sb.append(" " + columnInterval + " "
                        + makeValueAlign(columnNameList.get(i), maxFieldValueLengthMap.get(columnNameList.get(i))));
            }
        ...
        if (null != columnValueList && columnValueList.size() > 0) {
            ...
            for (int x = 0; x < columnValueList.size(); x++) {
                ...
                if (x > this.pringtTableRow) {
                    String info = "打印了" + String.valueOf(pringtTableRow) + "行;共有"
                            + String.valueOf(columnValueList.size()) + "行";
    ...
    /**
     * 获取一个字符串的标准长度是多少 A2*. 这样是四个标准长度 系统系统 这样是8个标准长度
     *
     * @param str
     * @return
     */
    ...
    private void setMaxFieldValueLengthMap(List<String> columnNameListTmp, List<List<String>> columnValueListTmp,
                                           Map<String, Integer> maxFieldValueLengthMapTmp) {
